...
void Manifold::RemoveExpiredContacts() {
	...
	for (int i = 0; i < m_numContacts; i++)
	{
		...
		// Calculate the tangential separation and the penetration depth
		const Vec3 ab = b - a;
		float penetrationDepth = normal.Dot(ab);
		Vec3 abNormal = normal * penetrationDepth;
		Vec3 abTengent = ab - abNormal;

		// If the tangential displacement is less than a specific threshold, it's ok to keep it
		const float distanceThreshold = 0.02f;
		if (abTengent.GetLengthSqr() < distanceThreshold * distanceThreshold && penetrationDepth <= 0.0f)
		{
			continue;
		}

		// This contact has moved beyound its treshold and should be removed
...
void Manifold::AddContact( const contact_t & contact_old ) {
	...
	int newSolt = m_numContacts;
	if (newSolt >= MAX_CONTACTS)
	{
		Vec3 avg = Vec3(0, 0, 0);
	...
	m_contacts[newSolt] = contact;
	m_constraints[newSolt].m_bodyA = contact.bodyA;
	m_constraints[newSolt].m_bodyB = contact.bodyB;
	m_constraints[newSolt].m_anchorA = contact.ptOnA_LocalSpace;
	m_constraints[newSolt].m_anchorB = contact.ptOnB_LocalSpace;

	// Get the normal in BodyA's space
	Vec3 normal = m_bodyA->m_orientation.Inverse().RotatePoint(contact.normal * -1.0f);
	m_constraints[newSolt].m_normal = normal;
	m_constraints[newSolt].m_normal.Normalize();

	m_constraints[newSolt].m_cachedLambda.Zero();

	if (newSolt == m_numContacts)
	{
		m_numContacts++;
	}
}
