...
Vec2 SignedVolume1D(const Vec3& s1, const Vec3& s2)
{
	...
	// Project the simplest point and project the origin onto the axis with greatest length
	const float a = s1[idx];
	const float b = s2[idx];
	const float p = p0[idx];

	// Get the signed distance from a to p and p to b
	const float C1 = p - a;
	const float C2 = b - p;
...
Vec3 SignedVolume2D(const Vec3& s1, const Vec3& s2, const Vec3& s3)
{
	...
	int x = (idx + 1) % 3;
	int y = (idx + 2) % 3;
	Vec2 s[3];					// our 2 points in the selected plan
	s[0] = Vec2(s1[x], s1[y]);
	s[1] = Vec2(s2[x], s2[y]);
	s[2] = Vec2(s3[x], s3[y]);
	Vec2 p = Vec2(p0[x], p0[y]);

	// Get the sub-areas of the triangles formed from the projection origin and the edges
	Vec3 areas;
	for (int i = 0; i < 3; i++)
	{
		int j = (i + 1) % 3;
...
====================================================
CompareSigns
	Return 1 if a and b have the same sign, 0 if not
====================================================
*/
...
point_t Support(const Body* bodyA, const Body* bodyB, Vec3 dir, const float bias)
{
	...
	// Find the point in B furthest in direction
	point.ptB = bodyB->m_shape->Support(dir, bodyB->m_position, bodyB->m_orientation, bias);

	// Return the point, in the mikowski sum, furthest in direction
...
Project the origin onto the simplex to acquire the new search direction,
also check if the origin is "inside" the simplex
Lambdas are the simplex point's positions
================================
*/
...
Checks wether the new point already exist in the simplex
================================
*/
...
bool GJK_DoesIntersect( const Body * bodyA, const Body * bodyB ) {
	...
	do
	{
		...
		// If the new point is the same as the previous point, then we can't extand any further
		...
		// If the new point hasn't moved passed the origin, then the origin cannot be in the set.
		// And therefore ther is no collision
		...
		// Check that the new projetion of the origin onto the simplex is closer than the previous
		...
		// Use the lambdas that support the new search direction, and invalidate any point that don't support it
...
This borrow our signed volume code to perform the barycentric coordinates.
================================
*/
Vec3 BarycentricCoordinates(Vec3 s1, Vec3 s2, Vec3 s3, const Vec3& pt)
{
	...
	// Find the axis with the greatest projection area
	int idX = 0;
	float area_max = 0;
	for (int i = 0; i < 3; i++)
	{
...
SignedDistanceToTriangle
	This function gives us the dot product of the normal of the triangle and the dist from pt to tri.a
	If the dist > 0 then the point in in front of the triangle
	Else it is inside or behind
================================
*/
...
int RemoveTrianglesFacingPoint(const Vec3& pt, std::vector<tri_t>& triangles, const std::vector<point_t>& points)
{
	...
	for (int i = 0; i < triangles.size(); i++)
	{
		...
		if (dist > 0.0f)
		{
			// This triangle face the point, remove it
...
================================
FindDanglingEdges
	Return an array of all the points that are owned by only 1 triangle
================================
*/
...
================================
EPA_Expand
	We have the simplex, so we know ther's an intersection, but we don't have any of the surface of C (C = mikoski diff build from A and -B)
	We need to find the surface on C that is the closest to the origin
	To do this, we measure the dist from the origin to all side of the simplex and we search for the closest
	Once we have the closest surface to the origin on C, we use the barycentric coords to calculate the points on A and B
================================
*/
float EPA_Expand(const Body* bodyA, const Body* bodyB, const float bias, const point_t simplexPoint[4], Vec3& ptOnA, Vec3& ptOnB)
{
	...
	for (int i = 0; i < 4; i++)
	{
		points.push_back(simplexPoint[i]);
		center += simplexPoint[i].xyz;
	}
	center *= 0.25f;
	...
	for (int i = 0; i < 4; i++)
	{
		...
		// The unused point is always on the negative/inside the triangle.. make sure the normal points away
		if (dist > 0.0f)
		{
	...
	while (1)
	{
		...
		// If w already exist, then just stop because it means we can't expand any further
	...
	// Get the projection of the closest origin on the closest triangle
	const int idx = ClosestTriangle(triangles, points);
...
void GJK_ClosestPoints( const Body * bodyA, const Body * bodyB, Vec3 & ptOnA, Vec3 & ptOnB ) {
	...
	do
	{
		...
		// If the new point is the same as the previous point, then we can't expand
		// any further
		if (HasPoint(simplexPoints, newPt))
		{
			break;
		...
		// Check that the new projection of the origin onto the simplex is clother than the previous
		float dist = newDir.GetLengthSqr();
		if (dist >= closestDist)
		{
			break;
...
bool GJK_DoesIntersect( const Body * bodyA, const Body * bodyB, const float bias, Vec3 & ptOnA, Vec3 & ptOnB ) {
	...
	do
	{
		...
		// If this new point hasn't moved passed the origin, then the origin cannot
		// be in the set. And therefore there is no collision
		float dotdot = newDir.Dot(newPt.xyz - origin);
		if (dotdot < 0.0f)
		{
			break;
	...
	//
	// Expand the simplex by the bias amounts
	//
