...
bool SphereSphereDynamic(const ShapeSphere* shapeA, const ShapeSphere* shapeB, const Vec3& posA, const Vec3& posB, const Vec3& velA, const Vec3& velB, const float dt, Vec3& ptOnA, Vec3& ptOnB, float& toi)
{
	...
	// if the collision is only in the past, then ter's no future collision this frame
	if (t1 < 0.0f)
	{
		return false;
	}

	// Get the earliest postive time of impact
	toi = (t0 < 0.0f) ? 0.0f : t0;

	// if the earliest collision is too far in the future, then ther's no collision this frame
	if (toi > dt)
	{
		return false;
	}
...
bool SphereSphereStatic(const ShapeSphere* shapeA, const ShapeSphere* shapeB, const Vec3& posA, const Vec3& posB, Vec3& ptOnA, Vec3& ptOnB)
{
	...
	ptOnA = posA + norm * shapeA->m_radius;
	ptOnB = posB - norm * shapeB->m_radius;

	const float radiusAB = shapeA->m_radius + shapeB->m_radius;
	const float lengthSquare = ab.GetLengthSqr();
	if (lengthSquare <= (radiusAB * radiusAB))
	{
		return true;
...
bool ConservativeAdvance(Body* bodyA, Body* bodyB, float dt, contact_t& contact)
{
	...
	while (dt > 0.0f)
	{
		...
		// project the relative velocity onto the ray of the shortest distance
		Vec3 relativeVelocity = bodyA->m_linearVelocity - bodyB->m_linearVelocity;
		float orthoSpeed = relativeVelocity.Dot(ab);

		//Add to the ortho speed the maximum angular speeds of the relative shapes
