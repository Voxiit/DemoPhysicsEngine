...
void ConstraintOrientation::PreSolve( const float dt_sec ) {
	// Get the world pasce position of the hinge from A's orientation
	const Vec3 worldAnchorA = m_bodyA->BodySpaceToWorldSpace(m_anchorA);

	// Get the world pasce position of the hinge from B's orientation
	...
	Mat4  P_T = P.Transpose(); // It's pointless to do this with this matrix implementation. But it's self commenting
