...
void ConstraintHingeQuat::PreSolve( const float dt_sec ) {
	...
	//
	// Apply war startinf from last frame
	//
	const VecN impulses = m_Jacobian.Transpose() * m_cachedLambda;
	ApplyImpulses(impulses);

	//
	// Calculate the baugmarte stabilization
	//
	float C = r.Dot(r);
...
void ConstraintHingeQuat::Solve() {
	...
	const MatMN invMassMatrix = GetInverseMassMatrix();
	const MatMN J_W_T = m_Jacobian * invMassMatrix * JacobianTranspose;
	VecN rhs = m_Jacobian * q_dt * -1.0f;
	rhs[0] -= m_baumgarte;

	// Solve for the lagrange multipliers
	const VecN lambdaN = LCP_GaussSeidel(J_W_T, rhs);
...
void ConstraintHingeQuatLimited::PreSolve( const float dt_sec ) {
	...
	// Check if ther's an angle violation
	...
	//
	// The primary distance constraint that holds the anchor points together
	//
	m_Jacobian.Zero();
...
void ConstraintHingeQuatLimited::Solve() {
	...
	// Solve for the lagrange multipliers
	VecN lambdaN = LCP_GaussSeidel(J_W_T, rhs);
