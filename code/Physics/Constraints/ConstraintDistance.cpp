...
void ConstraintDistance::PreSolve( const float dt_sec ) 
{
	...
	//
	// Calculate the baugmarte sabilization
	//
	float C = r.Dot(r);
...
{
	const MatMN JacombianTranspose = m_Jacobian.Transpose();

	// Build the system of equation
	const VecN q_dt = GetVelocities();
	const MatMN invMassMatrix = GetInverseMassMatrix();
	const MatMN J_W_JT = m_Jacobian * invMassMatrix * JacombianTranspose;
	...
	// Apply the impulses
	const VecN impulses = JacombianTranspose * lambdaN;
	ApplyImpulses(impulses);
